...
class MovingBarrier {
    ...
    setCollision(ball: Phaser.Physics.Arcade.Sprite) {
        this.scene.physics.add.collider(
            ball,
            this.sprite,
            this.handleCollision as unknown as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
            undefined,
            this
        )
        this.scene.physics.add.overlap(
            ball,
            this.sprite,
            this.handleOverlap as unknown as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
            undefined,
            this
        )
    }
...
class GameScene extends Phaser.Scene {
    ...
    private polygon: Phaser.Geom.Polygon | null = null
    ...
    create() {
        ...
        this.bg = this.add
            .image(0, 0, "bg")
            .setOrigin(0.5)
            .setPosition(sizes.width / 2 - 5, sizes.height / 2 + 10)
        ...
        this.polygon = new Phaser.Geom.Polygon(ponds["pond1"]["points"])

        this.polygon.points.forEach((point) => {
            point.x += sizes.width / 2 - 100
            point.y += sizes.height / 2 - 100
        })
        ...
        graphics.fillStyle(0x00bbff);
        graphics.fillPoints(this.polygon.points, true);


        const { left, top, width, height } = Phaser.Geom.Polygon.GetAABB(this.polygon);

        const boundingBox = this.add.zone(left, top, width, height).setOrigin(0, 0);

        this.physics.add.existing(boundingBox, true);

        this.physics.add.overlap(boundingBox, this.ball, undefined, function process(box, ball) {
            return Phaser.Geom.Polygon.ContainsPoint(this.polygon, new Phaser.Geom.Point((ball as Phaser.Types.Physics.Arcade.GameObjectWithBody).body.center.x, (ball as Phaser.Types.Physics.Arcade.GameObjectWithBody).body.center.y));
        })
        ...
        // score
        this.scoreText = this.add.text(sizes.width / 2 - 380, sizes.height / 2 - 310, "STROKE: " + this.stroke, {
            fontSize: "25px",
            color: "#000000",
            fontStyle: "bold",
    ...
    resize() {
        ...
        this.ball = this.physics.add
            .sprite(Math.round(sizes.width / 2 - 200), Math.round(sizes.height / 2 - 200), "ball")
            .setOrigin(0.5, 0.5)
        if (this.ball) {
            this.ball.setCollideWorldBounds(true)
            this.ball.setBounce(1)
    ...
    update() {
        ...
        if (this.movingBarrier && this.movingBarrier.graphics) {
            ...
            if (
                this.movingBarrier.graphics.x >=
                sizes.width -
                (sizes.width - sizes.width / 2 - 400) -
                this.movingBarrier.width -
                (sizes.width - sizes.width / 2 - 400)
            ) {
        ...
        if (this.ball && this.polygon) {
            if (Phaser.Geom.Polygon.ContainsPoint(this.polygon, new Phaser.Geom.Point((this.ball as Phaser.Types.Physics.Arcade.GameObjectWithBody).body.center.x, (this.ball as Phaser.Types.Physics.Arcade.GameObjectWithBody).body.center.y))) {
        ...
        if (this.ball && this.pond) {
            const distanceToPond = Phaser.Math.Distance.Between(this.ball.x, this.ball.y, this.pond.x, this.pond.y)
